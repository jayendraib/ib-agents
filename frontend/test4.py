...
# ========== CONFIGURATION ==========
FILE_PATH = "/home/jayendra/jayendraproject/sebiautomate/outputs/SEBIDebarred_Format.xlsx"
API_URL = "https://qa-kychecker.indiabonds.com/SEBI/FileUploadApi"
API_KEY = "0b80d0fe42644ae3a5a8277ccff4254b"
COMMENT = "AutomatedUpload"

# ========== STEP 1: READ & TRIM TO 10 ROWS ==========
...
print(f"Trimmed to: {len(df_10)} rows")

# ========== STEP 2: SAVE 10-ROW FILE TO MEMORY ==========
excel_buffer = BytesIO()
df_10.to_excel(excel_buffer, index=False, engine='openpyxl')
...
files = {
    "file": ("SEBIDebarred_Format_10rows.xlsx", excel_buffer, 
...
if "application/vnd.openxmlformats" in content_type or "application/octet-stream" in content_type:
    ...
    try:
        ...
        # Check for errors
        if data.get("result") == "ProcessedWithErrors" and data.get("errors"):
            print("\n❌ Errors found:")
            for err in data["errors"]:
                print(f"  PAN: {err.get('pan')} → {err.get('error')}")
            
            # Decode and save error file
            error_file_b64 = data.get("errorFile")
            if error_file_b64:
                # Decode base64
                zip_bytes = base64.b64decode(error_file_b64)
                
                # Save as zip first to inspect
                with open("error_file.zip", "wb") as f:
                    f.write(zip_bytes)
                print("✅ Saved error_file.zip")
                ...
                with zipfile.ZipFile(io.BytesIO(zip_bytes), 'r') as z:
                    ...
                    # Extract the Excel file (usually xl/workbook.xml or similar)
                    # Find the .xlsx file
                    excel_name = [name for name in z.namelist() if name.endswith('.xlsx') or 'workbook' in name]
                    if excel_name:
                        # Actually the zip IS the xlsx (OpenXML format)
                        # So just save it directly as .xlsx
                        with open("error_file.xlsx", "wb") as f:
                            f.write(zip_bytes)
                        print("✅ Saved error_file.xlsx")
                        
                        # Read and display
                        df = pd.read_excel("error_file.xlsx")
                        print(f"\n📊 Error file contents ({len(df)} rows):")
                        print(df.to_string())
                        
        elif data.get("result") == "OK" or data.get("success"):
            print("✅ 200 OK — No errors")
